#!/usr/bin/env python
# -*- coding: UTF-8 -*-
# @File ：article_ctrl.py
# @Author ：orange
# @Date ：2021/12/6 下午7:46
...
from back.service import QueryComponent, MakeupPost, query_maker, assert_new_tag_in_tags
from back.utils.date import date_maker
from back.service import tag_ctrl, categorie_ctrl, auth_ctrl

category_poster = categorie_ctrl.PostCategoryCtrl()
tag_poster = tag_ctrl.PostTagCtrl()
...
class GetArticles(object):
    ...
    @classmethod
    def article_order_by_data(desc=True):
        """
        按照日期排序
        :return:
        """
        sql_text = 'select * from mb_article order by create_date :desc;'
        articles = sql_get_data(sql_text=sql_text, args={'desc': desc})
        return articles
    ...
    def get_post_detail_by_args(self, query_by, order_by, category_id, tag_id, year, month, new=False, hot=False,
        ...
        if query_by:
            ...
            if query_by == 'category':
                query_data = query_maker.query_post_by_category_of(category_id, order_by=order_by, desc=order_by_desc)
            elif query_by == 'tag':
                query_data = query_maker.query_post_by_tag_of(tag_id, order_by=order_by, desc=order_by_desc)
    ...
    def get_article_detail_by_args(self, args: dict, order_by: str = 'create_date', cur_page: int = 1,
                                   page_size: int = 5, sort: str = 'desc', **kwargs) -> list:
        """
        根据各种条件查询文章
        :param args:
        :param order_by: 排序的字段
        :param cur_page: 页码
        ...
        """
        # 根据条件查询
        where_text = ' where mb_article.body_id is not null '
        order_by_text = ' order by '
        join_text = ''

        # todo 字符串替换的地方需要优化，有SQL注入的风险
        category = args.get('category')
        if category:
            where_text += f" and mb_article.category_id ={category} "
        tag = args.get('tag')
        if tag:
            join_text += f' left join mb_post_tags on mb_article.post_id = mb_post_tags.post_id '
            where_text += f" and mb_post_tags.tag_id = {tag} "
        archive = args.get('archive')
        if archive:
            year = args.get('year')
            month = args.get('month')
            where_text += f" and year(mb_article.create_date) = {year} and month(mb_article.create_date) = {month} "

        if args.get('new') or order_by == 'create_date':
            order_by_text += ' mb_article.create_date '
        elif args.get('hot') or order_by == 'view_counts':
            order_by_text += ' mb_article.view_counts '
        else:
            pass
        order_by_text += sort

        number = (cur_page - 1) * page_size
        sql_text = f"""
            select * from mb_article 
            {join_text}
            {where_text}
            {order_by_text}
        ...
        params = {}
        result_data = sql_get_data(sql_text=sql_text, args={})

        sql_count = f"""
            select count(*) as count_num from mb_article 
            {join_text}
            {where_text}
            {order_by_text}
        """
        count = sql_get_data(sql_text=sql_count, args={})
        # print('--sql_count--', sql_count)
        print('result_data: ', result_data)
        print('count: ', count)
        return result_data, count[0].get('count_num')


class PostArticleCtrl(object):
    """
    创建新博文
    """
...
article_get_ctrl = GetArticles()
article_post_ctrl = PostArticleCtrl()
article_put_ctrl = PutPostCtrl()
article_patch_ctrl = PatchPostCtrl()
article_del_ctrl = DelPostCtrl()
