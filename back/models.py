...
class User(db.Model):
    ...
    slogan = db.Column(db.String(64), default='这个人很懒，什么都没写！', comment='个性签名')
...
# 标签和文章为多对多关系，创建中间表
posts_tags_table = db.Table('mb_post_tags', db.Model.metadata,
                            db.Column('post_id', db.Integer, db.ForeignKey('mb_article.post_id')),
                            db.Column('tag_id', db.Integer, db.ForeignKey('mb_tag.id')),
...
class Article(db.Model):
    ...
    author_id = db.Column(db.Integer, db.ForeignKey('mb_user.id'), comment='作者id')
    body_id = db.Column(db.Integer, db.ForeignKey('mb_article_body.id'), unique=True, comment='文章结构体id')
    view_counts = db.Column(db.Integer, default='0', comment='文章阅读数')
    weight = db.Column(db.Integer, comment='置顶功能')
    category_id = db.Column(db.Integer, db.ForeignKey('mb_category.id'), comment='分类')
    create_date = db.Column(db.DateTime(), default=datetime.utcnow, comment='文章创建时间')
    # 注意，此处默认是标准时区，东八区需要修改配置，不修改的话显示会有差异
    update_date = db.Column(db.TIMESTAMP, default=db.text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'),
                            # 注意，此处默认是标准时区，东八区需要修改配置，不修改的话显示会有差距
                            comment='文章更新时间')
    '''
    ...
    tags = db.relationship('Tag', secondary=posts_tags_table, backref=db.backref('articles', lazy='dynamic'), lazy="dynamic")
    authors = db.relationship('User', back_populates='articles')
    categories = db.relationship('Category', back_populates='articles')
    ...
    def from_json(json_post):
        ...
        if body is None or body == '':
            raise ValueError('post dose not have a body.')
...
class ArticleBody(db.Model):
    ...
    文章结构体 表结构
    """
    __tablename__ = 'mb_article_body'
    __table_args__ = {'extend_existing': True}
    id = db.Column(db.Integer, primary_key=True, comment='主键')
    content_html = db.Column(db.Text, comment='文章的html')
    content = db.Column(db.Text, comment='文章内容')
    summary = db.Column(db.String(1000), default='上帝赠予的每一份礼物，都暗中标好了价格！', comment='文章摘要')
...
class Comment(db.Model):
    ...
    commented_post_id = db.Column(db.Integer, db.ForeignKey('mb_article.post_id'), comment='评论文章id')
    # 被回复id
    replied_id = db.Column(db.Integer, db.ForeignKey('mb_comment.id'), comment='回复id')
